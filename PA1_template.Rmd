...
First we unzip the archive and load the data, and then perform some prefpocessing:
...
We will then make a time series plot with this new dataframe. Since the `interval` variable had been converted to `factor`, by default `ggplot` will show all the 288 of its levels on the x-axis. This is neither pretty nor convenient, so we will adjust the appearence of the x-axis manually and only display the ticks at the beginning of each hour.
...
```
We can observe that after filling in missing values the total number of steps per day
